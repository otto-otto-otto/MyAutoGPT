...
async def requires_super_admin(
    ...
    """
    FastAPI dependency that requires a super admin user.

    First verifies the user has admin role, then checks the user's email
    against the configured admin_allowed_emails whitelist. If the whitelist
    is empty, all admin users are allowed (backward-compatible).

    Raises:
        HTTPException: 401 for authentication failures
        HTTPException: 403 for insufficient permissions or not in whitelist
    """
    # First verify admin role
    user = verify_user(jwt_payload, admin_only=True)

    # Lazy import to avoid circular dependency at module level
    from backend.util.settings import Settings

    settings = Settings()
    allowed_emails_str = settings.config.admin_allowed_emails
    if not allowed_emails_str or not allowed_emails_str.strip():
        # Whitelist is empty — allow all admin users (backward-compatible)
        return user
    ...
    raise fastapi.HTTPException(
        status_code=403,
        detail="Super admin access required — your account is not whitelisted",
    )
