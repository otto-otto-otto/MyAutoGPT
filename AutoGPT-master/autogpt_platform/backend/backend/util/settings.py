...
class Config(UpdateTrackingModel["Config"], BaseSettings):
    ...
    admin_allowed_emails: str = Field(
        default="",
        description=(
            "Comma-separated list of admin email addresses allowed to access"
            " the super-admin / ops panel. If empty, all admin-role users"
            " can access."
        ),
    )
    credits_per_usd: int = Field(
        default=100,
        description=(
            "Exchange rate: how many credits equal 1 USD of LLM cost."
            " Used to convert copilot chat token usage into credits charged."
            " e.g. 100 means $0.01 LLM cost = 1 credit."
        ),
    )
    credits_per_10000_tokens: float = Field(
        default=0.01,
        description=(
            "How many credits are deducted per 10,000 tokens consumed."
            " e.g. 0.01 means every 10,000 tokens = 0.01 credit (1 cent)."
        ),
    )
    ...
    deepseek_base_url: str = Field(
        ...
        description="Base URL for DeepSeek API (OpenAI-compatible endpoint)",
    )
    deepseek_fast_model: str = Field(
        default="deepseek-chat",
        description="Default DeepSeek fast model for chat completions",
    )
    deepseek_thinking_model: str = Field(
        default="deepseek-reasoner",
        description="Default DeepSeek reasoning model (R1)",
    )

    # Qianfan (Baidu ERNIE) API configuration
    qianfan_base_url: str = Field(
        default="https://qianfan.baidubce.com/v2",
        description="Base URL for Baidu Qianfan API (OpenAI-compatible endpoint)",
    )
    qianfan_default_model: str = Field(
        default="ernie-4.0-turbo-128k",
        description="Default Baidu Qianfan model for chat completions",
    )

    # DashScope (Alibaba Qwen) API configuration
    dashscope_base_url: str = Field(
        default="https://dashscope.aliyuncs.com/compatible-mode/v1",
        description="Base URL for Alibaba DashScope API (OpenAI-compatible endpoint)",
    )
    dashscope_default_model: str = Field(
        default="qwen-max",
        description="Default DashScope model for chat completions",
    )

    # Chinese search engine configuration
    baidu_search_enabled: bool = Field(
        default=True, description="Enable Baidu search engine"
    )
    sogou_search_enabled: bool = Field(
        default=True, description="Enable Sogou search engine"
    )
    ...
    chinese_token_ratio: float = Field(
        ...
        ge=1.0,
        le=4.0,
        description="Chinese character-to-token ratio for token estimation (default 1.8 chars/token)",
    )
    semantic_cache_enabled: bool = Field(
        default=True, description="Enable semantic cache for LLM responses"
    )
    fusion_strategy: str = Field(
        default="weighted_average",
        description="Multi-model fusion strategy: majority_voting, weighted_average, or consensus",
    )
...
class Secrets(UpdateTrackingModel["Secrets"], BaseSettings):
    ...
    qianfan_api_key: str = Field(default="", description="Baidu Qianfan API key")
    ...
    baidu_search_api_key: str = Field(default="", description="Baidu Search API key")
    sogou_search_api_key: str = Field(default="", description="Sogou Search API key")
