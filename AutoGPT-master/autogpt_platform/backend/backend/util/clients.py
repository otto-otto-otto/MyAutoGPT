...
def get_openai_client(*, prefer_openrouter: bool = False) -> "AsyncOpenAI | None":
    ...
    Prefers DeepSeek API key (primary Chinese LLM provider), falls back to
    openai_internal_api_key (direct OpenAI) and then open_router_api_key.
    """
    from openai import AsyncOpenAI

    deepseek_key = settings.secrets.deepseek_api_key
    ...
    if deepseek_key:
        return AsyncOpenAI(api_key=deepseek_key, base_url=DEEPSEEK_BASE_URL)
