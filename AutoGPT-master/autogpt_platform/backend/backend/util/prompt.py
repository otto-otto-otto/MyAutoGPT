...
def _normalize_model_for_tokenizer(model: str) -> str:
    ...
    Recognised OpenAI models (gpt-*, o1/o3/o4-*) use their own tokenizer;
    all other models (DeepSeek, Qianfan/Qwen, Anthropic/Claude, etc.) fall
    back to the ``o200k_base`` tokenizer via ``gpt-4o``.
    """
    if "/" in model:
        model = model.split("/")[-1]
    if any(
        known in model.lower()
        for known in ["gpt", "o1", "o3", "o4", "chatgpt", "text-"]
    ):
        return model
    # Non-OpenAI model — use gpt-4o tokenizer (o200k_base) which has the
    # widest vocabulary coverage and best handles Chinese/multilingual text.
    return "gpt-4o"
