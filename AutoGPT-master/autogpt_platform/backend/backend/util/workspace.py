...
class WorkspaceManager:
    ...
    async def rename_file(self, file_id: str, new_name: str) -> Optional[WorkspaceFile]:
        """
        Rename a workspace file.

        Updates both the ``name`` and ``path`` fields so the virtual path
        stays consistent with the new filename. The directory portion of
        the existing path is preserved.

        Args:
            file_id: The file's ID
            new_name: New filename (must not be empty or contain path separators)

        Returns:
            Updated WorkspaceFile, or None if not found or name conflict
        ...
        # Disallow path separators in filename for safety
        ...
        # Build new path: keep directory prefix, replace filename component
        old_path = file.path
        # Split into directory and filename
        if "/" in old_path:
            dir_prefix = old_path[: old_path.rfind("/") + 1]
        else:
            dir_prefix = "/"
        new_path = f"{dir_prefix}{new_name}"

        # Use the update function which will detect path conflicts
        return await db.update_workspace_file(
            file_id=file_id,
            workspace_id=self.workspace_id,
            name=new_name,
            path=new_path,
