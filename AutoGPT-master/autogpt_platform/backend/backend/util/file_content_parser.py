...
- **Word** (``.docx``) — pure-stdlib extraction via zipfile + xml → plain text
...
BINARY_FORMATS: frozenset[str] = frozenset({"parquet", "xlsx", "docx"})
...
def _parse_docx(content: bytes) -> str:
    ...
    text runs live at ``<w:t>`` elements inside ``<w:r>`` elements inside
    ``<w:p>`` (paragraph) elements.  This parser extracts those text nodes,
    joins them with space within each paragraph, and separates paragraphs
    with newlines.  No external dependency (python-docx) is required.
    """
    import xml.etree.ElementTree as ET

    # Pre-py3.11 ET.fromstring fails on non-bytes; BytesIO keeps it uniform.
    with zipfile.ZipFile(io.BytesIO(content)) as zf:
        if "word/document.xml" not in zf.namelist():
            raise ValueError("Not a valid .docx file: missing word/document.xml")
        xml_bytes = zf.read("word/document.xml")

    # Docx XML uses the namespace http://schemas.openxmlformats.org/wordprocessingml/2006/main
