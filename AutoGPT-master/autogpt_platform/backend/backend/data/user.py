...
async def get_or_create_user(user_data: dict) -> User:
    try:
        ...
        if not user:
            ...
            # Grant initial credits to new users (idempotent via transaction_key).
            if settings.config.enable_credit:
                try:
                    from backend.data.credit import UserCredit as _UserCredit

                    credit_system = _UserCredit()
                    await credit_system.grant_credits(
                        user_id=user_id,
                        amount=500,
                        reason="Initial sign-up grant",
                        transaction_key=f"INITIAL_GRANT-{user_id}",
                    )
                except Exception:
                    logger.exception(
                        "Failed to grant initial credits to user %s", user_id
                    )
