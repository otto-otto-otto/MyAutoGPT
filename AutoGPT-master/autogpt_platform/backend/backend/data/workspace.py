...
async def update_workspace_file(
    ...
    """
    Update a workspace file's name, path, and/or metadata.

    Only non-None fields are updated. When ``path`` is updated and the new
    path already exists for a different file, ``None`` is returned so the
    caller can treat that as a conflict.

    Args:
        file_id: The file ID
        workspace_id: Workspace ID for scoping
        name: New filename (optional)
        path: New virtual path (optional)
        metadata: New metadata dict; merged with existing (optional)

    Returns:
        Updated WorkspaceFile, or None if not found or path conflict
    """
    from prisma.errors import UniqueViolationError

    # Verify the file exists and belongs to the workspace
    file = await get_workspace_file(file_id, workspace_id)
    if file is None:
        return None
    ...
    if metadata is not None:
        ...
        merged = {**(file.metadata or {}), **metadata}
        update_data["metadata"] = SafeJson(merged)

    if not update_data:
        return file  # Nothing to update
    ...
    try:
        ...
        logger.info(
            f"Updated workspace file {file_id}: {list(update_data.keys())}"
        )
    except UniqueViolationError:
        # Path conflict — the new path already belongs to another file
        logger.warning(
            f"Path conflict updating workspace file {file_id} "
            f"to path {path}"
        )
        return None
