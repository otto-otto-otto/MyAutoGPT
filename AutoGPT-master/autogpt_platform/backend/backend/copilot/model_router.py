...
LD payload shape::
    ...
    {
      "fast":     {"standard": "anthropic/claude-sonnet-4-6", "advanced": "anthropic/claude-opus-4-6", "deepseek": "deepseek-chat"},
      "thinking": {"standard": "moonshotai/kimi-k2.6",         "advanced": "anthropic/claude-opus-4-6", "deepseek": "deepseek-reasoner"}
    }
...
ModelTier = Literal["standard", "advanced", "deepseek"]


def _config_default(config: ChatConfig, mode: ModelMode, tier: ModelTier) -> str:
    if mode == "fast":
        if tier == "deepseek":
            return config.fast_deepseek_model
        return (
    ...
    # mode == "thinking"
    if tier == "deepseek":
        return config.thinking_deepseek_model
    return (
