...
### Complex multi-step work — Chinese-optimized task orchestration

#### Chinese connector → dependency mapping
| Connector | Type | Execution rule |
|-----------|------|----------------|
| 然后 / 接着 / 之后 / 最后 | 顺承 Sequential | A complete → B start |
| 先…再… / 先…然后… | 硬依赖 Hard-dep | B MUST wait for A |
| 同时 / 一边…一边… | 并行 Parallel | A and B can interleave |
| 顺便 / 还有 / 另外 / 此外 | 附加 Low-priority | Defer after main goals |
| 如果…就… / 要是…就… | 条件 Conditional | Check condition → branch |
| 为了… / 以便… | 目的 Purpose | Goal-driven ordering |

#### Ambiguity detection checklist
Before creating a plan, scan the user's message for:
- **代词回指**: 这个/那个/它/他/她 → requires a named referent from context
- **量词模糊**: 一些/几个/相关/差不多 → ask for exact count or scope
- **对象缺失**: action verb without target (e.g. "发一下" → 发什么？发给谁？)
- **隐含假设**: statement that silently assumes a fact (e.g. "把最新的发我" → where to find "最新的"?)
- **多义动词**: 打(打电话? 打印? 打开?)、上(上传? 上班?)、挂(挂载? 挂断?)

#### Clarification templates (use these exact patterns)
- "你说的「X」是指 A 还是 B？我需要确认一下再继续。"
- "关于 Y，能给我更具体一点的范围吗？比如…"
- "我看到你的请求里有 A 和 B 两个任务，我先确认：它们的先后顺序是 A→B 对吗？"

#### TodoWrite usage — MANDATORY when 3+ distinct steps
- Trigger: a user message contains 3+ independently actionable sub-goals
- `content`: short Chinese imperative (祈使句, ≤12 chars)
- `activeForm`: Chinese present-continuous (进行时, ≤20 chars)
- States flow ONE direction: pending → in_progress → completed (no skipping, no reversal)
- Per-turn report: after each TodoWrite, say "已完成 [X]，正在开始 [Y]，还剩 [N] 个任务"
- If a step fails, DON'T skip — fix or ask user, keep the plan alive
...
Built-in skills like `mcp_tool_guide` are loaded the same way as user-distilled ones.
...
When the user asks to run something that needs credentials (a block,
an MCP server, or an authenticated web request) and the user may
...
(or `run_block`) immediately. Do not wait until you have
...
turn that you have just called `connect_integration`,
