...
async def _resolve_baseline_model(
    model: CopilotLlmModel | None, user_id: str | None
) -> tuple[str, str]:
    """Pick the model for the baseline path based on the per-request selection.

    Parses ``"provider:tier"`` compound string (or legacy bare tier name).
    Returns ``(provider, resolved_model_name)``.

    For DeepSeek, delegates to :func:`copilot.model_router.resolve_model`
    so the ``(fast, tier)`` cell is LD-overridable per user.
    For Qwen/ERNIE, resolves directly from config.
    ``None`` maps to ``("deepseek", deepseek-chat)``.
    """
    from backend.copilot.service import parse_model_string

    provider, tier = parse_model_string(model)

    if provider == "deepseek":
        resolved = await resolve_model("fast", tier, user_id, config=config)
    else:
        resolved = config.get_provider_model(provider, tier)
...
# Model providers / families that accept ``image_url`` content blocks in the
# OpenAI-compatible chat format.  Chinese LLMs (DeepSeek, Qwen-family via
# Qianfan/DashScope, Kimi/K2 via OpenRouter) only accept ``text``-type
# content and 400 on ``image_url`` — embedding images for those providers
# crashes the entire request.
_VISION_CAPABLE_PROVIDERS: frozenset[str] = frozenset(
    {
        "openai/",
        "anthropic/",
        "google/",
...
def _model_supports_vision(model: str) -> bool:
    """Return True when *model* accepts OpenAI-style ``image_url`` content.

    Uses a provider-prefix allow-list.  Chinese models (``deepseek/*``,
    ``moonshotai/*``, ``qwen/*``, etc.) return False — embedding images
    for those providers causes a 400 ``unknown variant image_url`` error.
    """
    lowered = model.lower()
    # Anthropic direct model names (no vendor prefix) also support vision.
    if "claude" in lowered:
        return True
    return any(lowered.startswith(prefix) for prefix in _VISION_CAPABLE_PROVIDERS)
...
async def _analyze_images_with_vision(
    ...
    """Pre-process images through Qwen-VL and return structured text descriptions.

    Called when the main model does NOT support vision (e.g. DeepSeek).  Each
    ``image_block`` is converted from Anthropic format to OpenAI ``image_url``
    format and sent to Qwen-VL for analysis.  The resulting text descriptions
    are injected into the user message so the main model can reason about image
    content.

    Returns ``None`` on any failure (missing credentials, API error, timeout)
    so the caller can fall back to the existing text-note behaviour.
    """
    if not config.vision_enabled:
        logger.info("[Vision] Vision pre-processing is disabled (CHAT_VISION_ENABLED=false).")
        return None

    vision_api_key = config.vision_api_key
    if not vision_api_key:
        logger.info(
            "[Vision] No vision API key configured — skipping image analysis. "
            "Set CHAT_VISION_API_KEY or DASHSCOPE_API_KEY to enable."
        )
        return None
    ...
    try:
        ...
        # Build a prompt that guides Qwen-VL to produce structured, useful
        # descriptions for the downstream main model.
        analysis_prompt = (
            "Please describe the following image(s) in detail. "
            "Focus on the key visual elements: text, objects, layout, "
            "colors, charts, diagrams, UI elements, people, or any "
            "information that would help a text-only assistant understand "
            "what is in the image.  Be thorough but concise."
        )
        if user_text:
            analysis_prompt += (
                f"\n\nThe user asked: \"{user_text}\". "
                "Tailor your description to help answer this question."
            )

        vision_content.append({"type": "text", "text": analysis_prompt})

        # Convert Anthropic-format image_blocks to OpenAI image_url format.
        ...
        if not description.strip():
            logger.warning("[Vision] Qwen-VL returned empty description for %d image(s).", len(image_blocks))
            return None

        logger.info(
            "[Vision] Analysed %d image(s) in %.1fs using %s — %d chars.",
            len(image_blocks), elapsed, config.vision_model, len(description),
        )

        noun = "image" if len(image_blocks) == 1 else "images"
        result = (
            f"\n\n[Image Analysis Results — {len(image_blocks)} {noun} "
            f"analysed by {config.vision_model}]\n{description}"
        )
        return result

    except Exception:
        logger.warning(
            "[Vision] Image analysis failed for %d image(s) — "
            "falling back to text-only note.",
            len(image_blocks),
            exc_info=True,
        )
        return None
...
async def _baseline_llm_caller(
    ...
    *,
    ...
    round_text = ""
    try:
        client = _get_main_client(provider=state.provider)
...
async def stream_chat_completion_baseline(
    ...
    **_kwargs: Any,
    ...
    provider, resolved_model = await _resolve_baseline_model(model, user_id)
    try:
        ...
        from backend.copilot.service import parse_model_string as _parse_model

        _, _tier = _parse_model(model)
        tier_default = (
                ...
                if isinstance(existing, str):
                    ...
                    if image_blocks:
                        if _model_supports_vision(active_model):
                            # Vision-capable model — embed images as content parts.
                            ...
                            # Non-vision model (DeepSeek, Kimi, Qwen, etc.) —
                            # image_url blocks would 400.  Try Qwen-VL pre-processing
                            # first; fall back to a text note on failure.
                            img_count = len(image_blocks)

                            # Extract the user's original question for context-aware
                            # vision analysis.
                            original_user_text = (
                                openai_messages[i].get("content", "")
                                if isinstance(openai_messages[i].get("content"), str)
                                else ""
                            )
                            ...
                            if vision_result:
                                openai_messages[i]["content"] = text + vision_result
                            else:
                                # Fallback: keep the existing text-note behaviour.
                                note = (
                                    f"\n[Note: {img_count} attached image(s) could not "
                                    f"be displayed inline because the current model "
                                    f"({active_model}) does not support vision. "
                                    f"File metadata (name, type, size) is listed "
                                    f"above. Use read_workspace_file with the "
                                    f"corresponding file_id to access images.]"
                                )
                                openai_messages[i]["content"] = text + note
                    else:
    ...
    state = _BaselineStreamState(model=active_model, provider=provider)
    ...
    try:
        ...
        await persist_and_record_usage(
            ...
            model=active_model,
            provider="open_router" if config.openrouter_active else "deepseek",
        )
        await spend_copilot_credits(
            user_id=user_id,
            cost_usd=state.cost_usd,
            total_tokens=uncached_prompt + state.turn_completion_tokens,
            reason="copilot_chat_turn",
            model=active_model,
            log_prefix="[Baseline]",
        )
