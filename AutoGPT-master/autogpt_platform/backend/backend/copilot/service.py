...
from .token_tracking import _extract_cache_creation_tokens, persist_and_record_usage, spend_copilot_credits
...
def resolve_chat_model(tier: CopilotLlmModel | None) -> str:
    ...
    Accepts both compound strings (``"deepseek:advanced"``) and legacy
    bare tiers (``"advanced"``).
    """
    _, tier_name = parse_model_string(tier)
    if tier_name == "advanced":
        return config.thinking_advanced_model
    return config.thinking_standard_model


def parse_model_string(model: str | None) -> tuple[str, str]:
    """Parse a compound model string ``"provider:tier"`` into ``(provider, tier)``.

    Backward-compatible with old values:
    - ``"advanced"`` → ``("deepseek", "advanced")``
    - ``"standard"`` → ``("deepseek", "standard")``
    - ``"qwen:advanced"`` → ``("qwen", "advanced")``
    - ``None`` → ``("deepseek", "standard")``
    """
    if not model:
    ...
    # Legacy: bare "standard" / "advanced"
    return ("deepseek", model if model in ("standard", "advanced") else "standard")


_provider_clients: dict[str, LangfuseAsyncOpenAI] = {}
_aux_client: LangfuseAsyncOpenAI | None = None
_vision_client: LangfuseAsyncOpenAI | None = None
_langfuse = None


def _get_main_client(provider: str = "deepseek") -> LangfuseAsyncOpenAI:
    """OpenAI-compat client for a given LLM provider.

    Clients are cached per-provider in ``_provider_clients``.
    ...
    if provider not in _provider_clients:
        api_key, base_url = config.get_provider_credentials(provider)
        _provider_clients[provider] = LangfuseAsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
            timeout=300.0,
            max_retries=0,
...
def _get_aux_client() -> LangfuseAsyncOpenAI:
    ...
    if _aux_client is None:
        ...
        _aux_client = LangfuseAsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
            timeout=300.0,
            max_retries=0,
...
def _get_vision_client() -> LangfuseAsyncOpenAI:
    """Qwen-VL vision client for image pre-processing.

    Independent of the main/aux clients — uses ``config.vision_client_credentials``
    so a deployment can point it at DashScope without affecting the main
    DeepSeek conversation client.  Longer timeout (120 s) to accommodate
    image analysis latency.
    """
    global _vision_client
    if _vision_client is None:
        api_key, base_url = config.vision_client_credentials
        _vision_client = LangfuseAsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
            timeout=120.0,
            max_retries=1,
...
def reset_clients() -> None:
    ...
    global _provider_clients, _aux_client
...
_CACHEABLE_SYSTEM_PROMPT = f"""You are AutoPilot, a helpful, versatile AI assistant powered by a large language model. Always identify yourself simply as AutoPilot when asked about your identity.

Your goal is to help users by:
- Understanding their needs, questions, and business context
- Providing accurate, actionable answers and solutions
...
## Multi-Goal Task Decomposition (CRITICAL)

When a user request contains multiple distinct goals — signalled by Chinese connectors like 然后、同时、顺便、还有、另外、接着、之后、最后、先…再…, or by comma/semicolon-separated independent clauses — you MUST decompose BEFORE acting.

### Decomposition Protocol

1. **Ambiguity check** — Before extracting goals, scan for:
   - **Dangling references**: 这个、那个、它、他、她、这些、那些 — when the referent is unclear from context, ask: "你提到的「X」是指…还是…？"
   - **Scope ambiguity**: 相关文件、那些东西、类似的 — nail down the exact scope.
   - **Missing preconditions**: Actions that require unspecified inputs or credentials — surface the gap.
   - **Implicit assumptions**: A goal that assumes a prior step the user didn't state — make it explicit and ask for confirmation.
   → If ANY ambiguity found, STOP and clarify first. Do NOT decompose until resolved.

2. **Goal extraction** — Parse the user's message into distinct, independent sub-goals. Each sub-goal must be a single, self-contained actionable intent. Strip connecting words and extract the core verb phrase.

3. **Dependency ordering** — Sort sub-goals:
   - `先…再…` / `然后` → sequential dependency (A must finish before B)
   - `同时` / `顺便` / `还有` → parallel (independent, can interleave)
   - Implicit dependency (e.g. B needs A's output) → treat as sequential
   - Assign priorities: urgent/blocking > normal > nice-to-have

4. **TodoWrite to commit** — Write the ordered plan via `TodoWrite` with these rules:
   - `content`: short Chinese imperative (e.g. "查询今日天气")
   - `activeForm`: present-continuous Chinese (e.g. "正在查询今日天气")
   - Exactly ONE item `in_progress` at any time
   - Mark completed IMMEDIATELY after finishing a step; advance the next pending to in_progress in the same TodoWrite call
   - After ALL items completed, announce the summary in one sentence

### Examples

User: "帮我查一下深圳天气，然后把结果发邮件给老板，顺便整理这个月的销售数据"

→ Ambiguity: 老板的邮箱？这个月=哪个月？
→ Clarify first, then decompose into: ① 查天气 → ② 发邮件 → ③ 整理销售数据

User: "下载这份报告，分析数据，写总结"

→ No ambiguity → TodoWrite:
1. [pending] 下载报告
2. [pending] 分析数据
3. [pending] 写总结
...
For users you are meeting for the first time with no context provided, greet them warmly and ask how you can help."""
...
async def _record_title_generation_cost(
    *,
    ...
    await spend_copilot_credits(
        user_id=user_id,
        cost_usd=cost_usd,
        total_tokens=uncached_prompt + completion_tokens,
        reason="copilot_title_generation",
        model=model,
        log_prefix="[title]",
    )
