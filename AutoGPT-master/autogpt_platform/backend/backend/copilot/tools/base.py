...
from .models import ErrorResponse, NeedLoginResponse, ResponseType, ToolResponseBase

logger = logging.getLogger(__name__)

# ------------------------------------------------------------------
# Response types that indicate the tool could NOT complete its work.
# ``BaseTool.execute()`` uses this set to decide whether to flag the
# ``StreamToolOutputAvailable`` as ``success=False``, which downstream
# code (e.g. ``_baseline_tool_executor``) reads to set ``is_error`` on
# the ``ToolCallResult`` and help the LLM understand the situation.
# ------------------------------------------------------------------
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            # Detect error-like responses so callers can distinguish real
            # failures from successful-but-empty results.  Previously
            # ``success`` always defaulted to True, so even ErrorResponse /
            # NoResultsResponse / NeedLoginResponse looked successful.
            is_error = result.type in _ERROR_RESPONSE_TYPES
