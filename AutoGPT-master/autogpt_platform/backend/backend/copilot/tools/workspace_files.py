...
# MIME types for documents that can have their text extracted.
_DOCX_MIME_TYPES = {
...
def _is_docx_mime(mime_type: str) -> bool:
    """Check if *mime_type* is a Word document that we can extract text from."""
    return mime_type in _DOCX_MIME_TYPES


def _extract_docx_text(file_bytes: bytes) -> str:
    """Extract plain text from a .docx file using only the stdlib.

    See :func:`backend.util.file_content_parser._parse_docx` for the
    canonical implementation — this copy exists here to keep
    ``workspace_files.py`` self-contained for the tool's inline-text
    path without importing from the ``@@agptfile:`` parser module.
    """
    ...
    async def _execute(
        ...
        **kwargs,
        ...
        try:
            ...
            is_docx = _is_docx_mime(file_info.mime_type)
            ...
            if char_offset > 0 or char_length is not None:
                ...
                if is_docx:
                    try:
                        ...
                        return ErrorResponse(
                            message=f"Failed to extract text from {file_info.name} "
                            f"(not a valid .docx file).",
                            session_id=session_id,
                        )
                else:
            ...
            # Inline content for small text/image/docx files
            if is_small and (is_text or is_image or is_docx) and not force_download_url:
                ...
                if is_docx:
                    try:
                        ...
                        # Fall back to raw base64 if extraction fails
            ...
            if is_text or is_docx:
                try:
                    raw = cached_content or await manager.read_file_by_id(
                        target_file_id
                    )
                    if is_docx:
                        preview = _extract_docx_text(raw)
                        if len(preview) > self.PREVIEW_SIZE:
                            preview = preview[: self.PREVIEW_SIZE] + "..."
                    else:
                        preview = raw[: self.PREVIEW_SIZE].decode(
