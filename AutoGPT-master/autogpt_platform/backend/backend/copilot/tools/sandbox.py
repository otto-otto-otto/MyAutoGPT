...
def _build_bwrap_command(
    ...
    *,
    ...
    When *use_user_namespace* is ``False``, ``--unshare-user`` is omitted.
    This is a fallback for container environments where user-namespace
    creation is not permitted (e.g. restricted Docker, CI runners, etc.).
    The result is weaker isolation but still provides filesystem separation,
    environment cleanup, network isolation, and resource limits.
    """
    cmd = [
        "bwrap",
    ]
...
async def run_sandboxed(
    ...
    Tries the full sandbox first (including ``--unshare-user``).  If
    bubblewrap fails with a user-namespace permission error (common in
    restricted container environments), automatically retries without
    ``--unshare-user`` — still providing filesystem separation, env
    cleanup, network isolation, and resource limits.
    ...
    # First attempt: full sandbox with user namespace
    full_command = _build_bwrap_command(command, cwd, safe_env)

    try:
        result = await _run_bwrap_command(full_command, cwd, timeout, safe_env)
        # If the stderr indicates a namespace permission error and we used
        # --unshare-user, retry without it.
        stderr_lower = result[1].lower()
        if any(
            ...
            logger.warning(
                "bwrap failed with namespace permission error; "
                "retrying without --unshare-user "
                "(stderr: %s)",
                result[1][:200],
            )
            fallback_cmd = _build_bwrap_command(
                command, cwd, safe_env, use_user_namespace=False
            )
            return await _run_bwrap_command(fallback_cmd, cwd, timeout, safe_env)
        return result
    except Exception:
        # If the initial attempt fails outright (process error), also try
        # without user namespace as a last resort.
        try:
            logger.warning(
                "bwrap full sandbox failed; retrying without --unshare-user"
            )
            fallback_cmd = _build_bwrap_command(
                command, cwd, safe_env, use_user_namespace=False
            )
...
async def _run_bwrap_command(
    full_command: list[str],
    cwd: str,
    timeout: int,
    safe_env: dict[str, str],
) -> tuple[str, str, int, bool]:
    """Execute a bubblewrap command and return (stdout, stderr, exit_code, timed_out)."""
    try:
