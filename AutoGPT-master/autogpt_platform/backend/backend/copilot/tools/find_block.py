...
class FindBlockTool(BaseTool):
    ...
    @staticmethod
    def _is_excluded(block) -> bool:
        """Check if a block is excluded from CoPilot usage (type or ID)."""
        return (
    ...
    @staticmethod
    def _build_block_summary(block, *, include_schemas: bool = False) -> BlockInfoSummary:
        """Build a BlockInfoSummary from a block instance.

        Extracted to avoid duplicating this logic in UUID lookup and search paths.
        """
        ...
        try:
            ...
            if is_uuid(query):
                ...
                if block:
                    ...
                    summary = self._build_block_summary(
                        block, include_schemas=include_schemas
                    )
            ...
            for result in results:
                ...
                if not for_agent_generation and self._is_excluded(block):
                    continue
            ...
            if blocks and len(blocks) < _TARGET_RESULTS:
                logger.debug(
                    f"find_block returned {len(blocks)}/{_TARGET_RESULTS} results "
                    f"for query '{query}' "
                    f"(filtered {len(results) - len(blocks)} excluded/disabled blocks)"
                )
