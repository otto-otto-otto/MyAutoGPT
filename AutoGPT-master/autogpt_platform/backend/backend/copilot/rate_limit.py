...
async def reset_daily_token_usage(user_id: str, daily_token_limit: int = 0) -> bool:
    """Reset a user's daily token usage counter in Redis.

    Called after a user pays credits to extend their daily token limit.
    Also reduces the weekly token counter by ``daily_token_limit`` tokens
    (clamped to 0) so the user effectively gets one extra day's worth of
    weekly capacity.

    Args:
        user_id: The user's ID.
        daily_token_limit: The configured daily token limit.
            When positive, the weekly counter is reduced by this amount.

    Returns False if Redis is unavailable so the caller can handle
    compensation (fail-closed for billed operations).
    """
    now = datetime.now(UTC)
    try:
        redis = await get_redis_async()
        ...
        w_key = _weekly_token_key(user_id, now=now) if daily_token_limit > 0 else None

        await redis.delete(d_key)
        if w_key is not None:
            await _decr_counter_floor_zero(redis, w_key, daily_token_limit)

        logger.info("Reset daily token usage for user %s", user_id[:8])
        return True
    except (RedisError, RedisClusterException, ConnectionError, OSError):
        logger.warning("Redis unavailable for resetting daily token usage")
        return False
...
# ---------------------------------------------------------------------------
# Token-based rate limiting (parallel to cost-based microdollar counters)
# ---------------------------------------------------------------------------
#
# The legacy ``copilot:cost`` counters track real USD spend.  Token counters
# track raw token consumption and are used as the primary user-facing rate
# limit for copilot conversations.  Both counter families coexist so
# deployments can switch between them via config flags.
#
...
async def record_token_usage(
    ...
    """Record a user's token consumption against daily and weekly counters.

    Fail-open semantics: Redis errors are logged but never re-raised, so
    a transient Redis blip does not block the user.  The pre-turn check
    (:func:`check_token_rate_limit`) already fails closed independently.

    Args:
        user_id: The user's ID.
        token_count: Number of tokens consumed (prompt + completion).
            Non-positive values are ignored.
    """
    ...
    logger.info("Recording token usage: %d tokens", token_count)
    ...
    # Persist token usage to PostgreSQL asynchronously so the counter
    # survives Redis restarts.  Fire-and-forget — errors are logged but
    # never block the user.
    _schedule_token_persist(user_id, token_count)
...
async def _persist_token_usage(
    ...
    """UPSERT token usage into PostgreSQL.

    Called asynchronously from ``record_token_usage()`` so this write
    does not add latency to the LLM turn path.  Errors are logged but
    never re-raised — the pre-turn check path already reads from
    PostgreSQL as a fallback, so a transient DB outage merely delays
    the durable counter update by one turn.
    """
    now = datetime.now(UTC)
    daily_date = now.strftime("%Y-%m-%d")
    year, week, _ = now.isocalendar()
    weekly_date = f"{year}-W{week:02d}"

    try:
        await ensure_db_connected()

        # UPSERT daily row with atomic increment.
        await prisma.execute_raw(
            '''
            INSERT INTO "platform"."UserTokenUsage"
                ("userId", "window", "dateKey", "tokens", "updatedAt")
            VALUES ($1, 'daily', $2, $3, NOW())
            ON CONFLICT ("userId", "window", "dateKey")
            DO UPDATE SET "tokens" = "platform"."UserTokenUsage"."tokens" + $3,
                          "updatedAt" = NOW()
            ''',
            user_id,
            daily_date,
        ...
        # UPSERT weekly row with atomic increment.
        await prisma.execute_raw(
            '''
            INSERT INTO "platform"."UserTokenUsage"
                ("userId", "window", "dateKey", "tokens", "updatedAt")
        ...
        logger.warning(
            "Failed to persist token usage to PostgreSQL (user=%s, tokens=%d)",
            user_id[:8],
            token_count,
            exc_info=True,
        )
...
def _schedule_token_persist(user_id: str, token_count: int) -> None:
    """Fire-and-forget the PostgreSQL token persist.

    Uses ``asyncio.create_task`` so the caller's response path is not
    blocked by the database round-trip.
    """
    try:
        asyncio.create_task(_persist_token_usage(user_id, token_count))
    except RuntimeError:
        # No running event loop (e.g. in a sync test / teardown path).
        logger.debug(
            "Cannot schedule token persist — no running event loop"
        )


async def _read_token_usage_fallback(
    user_id: str,
) -> tuple[int, int]:
    """Read current-window token usage from PostgreSQL.

    The authoritative source when Redis counters are empty (restart /
    TTL expiry / brown-out).  Returns ``(daily_tokens, weekly_tokens)``.
    Errors fall back to ``(0, 0)`` — the pre-turn check path treats
    an unreachable DB identically to a cold start.
    """
    now = datetime.now(UTC)
    daily_date = now.strftime("%Y-%m-%d")
    year, week, _ = now.isocalendar()
    ...
    try:
        ...
        rows = await prisma.usertokenusage.find_many(
            where={
                "userId": user_id,
                "dateKey": {"in": [daily_date, weekly_date]},
            },
        )
        ...
        for row in rows:
            if row.window == "daily":
                daily = row.tokens
            elif row.window == "weekly":
                weekly = row.tokens

        return daily, weekly
    except Exception:
        logger.warning(
            "Failed to read token usage from PostgreSQL fallback (user=%s)",
            user_id[:8],
            exc_info=True,
        )
        return 0, 0
...
async def get_token_usage_status(
    ...
    """Get current token-based usage status for a user.

    Args:
        user_id: The user's ID.
        daily_token_limit: Max tokens per day. 0 means no limit configured
            (window hidden from UI).
        weekly_token_limit: Max tokens per week. 0 means no limit configured
            (window hidden from UI).
        tier: The user's rate-limit tier.

    Returns:
        CoPilotUsageStatus with current token usage and limits.
    """
    now = datetime.now(UTC)
    daily_used = 0
    weekly_used = 0
    try:
        ...
        logger.warning("Redis unavailable for token usage status, returning zeros")

    # When Redis counters are zero (cold start after restart), fall back
    # to PostgreSQL to restore the durable counter values.
    if daily_used == 0 and weekly_used == 0:
        pg_daily, pg_weekly = await _read_token_usage_fallback(user_id)
        daily_used = max(daily_used, pg_daily)
        weekly_used = max(weekly_used, pg_weekly)
        # Warm the Redis cache so the next read is fast.
        if daily_used > 0 or weekly_used > 0:
            try:
                redis = await get_redis_async()
                d_key = _daily_token_key(user_id, now=now)
                w_key = _weekly_token_key(user_id, now=now)
                daily_ttl = max(
                    int((_daily_reset_time(now=now) - now).total_seconds()), 1
                )
                weekly_ttl = max(
                    int((_weekly_reset_time(now=now) - now).total_seconds()), 1
                )
                await asyncio.gather(
                    redis.set(d_key, daily_used, ex=daily_ttl),
                    redis.set(w_key, weekly_used, ex=weekly_ttl),
                )
            except (RedisError, RedisClusterException, ConnectionError, OSError):
                logger.debug("Redis cache warm failed after PostgreSQL fallback")
...
async def check_token_rate_limit(
    ...
    """Check if user is within token-based rate limits.

    Fails closed (raises RateLimitUnavailable) when Redis is unreachable.

    Args:
        user_id: The user's ID.
        daily_token_limit: Max tokens per day. 0 disables daily check.
        weekly_token_limit: Max tokens per week. 0 disables weekly check.

    Raises:
        RateLimitExceeded: When the user has exceeded their token limit.
        RateLimitUnavailable: When Redis is unreachable.
    """
    ...
    try:
        ...
        logger.warning("Token rate limit state unreadable, rejecting request: %s", exc)
        raise RateLimitUnavailable() from exc

    # When Redis returns zero for both windows (cold start after restart),
    # fall back to PostgreSQL.  If PostgreSQL is also unreachable we still
    # fail closed — we can't prove the user is under their cap.
    if daily_used == 0 and weekly_used == 0:
        try:
            pg_daily, pg_weekly = await _read_token_usage_fallback(user_id)
        except Exception:
            pg_daily, pg_weekly = 0, 0
        daily_used = max(daily_used, pg_daily)
        weekly_used = max(weekly_used, pg_weekly)
        # Warm the Redis cache so subsequent checks hit the cache.
        if daily_used > 0 or weekly_used > 0:
            try:
                d_key = _daily_token_key(user_id, now=now)
                w_key = _weekly_token_key(user_id, now=now)
