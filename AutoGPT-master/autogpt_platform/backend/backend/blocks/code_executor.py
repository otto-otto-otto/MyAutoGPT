...
class BaseE2BExecutorMixin:
    ...
    # Chinese locale setup commands for sandbox environment
    CHINESE_LOCALE_SETUP = [
        "export LANG=zh_CN.UTF-8",
        "export LC_ALL=zh_CN.UTF-8",
        "export PYTHONIOENCODING=utf-8",
    ]

    def _merge_setup_with_locale(
        self,
        setup_commands: Optional[list[str]] = None,
        enable_chinese_locale: bool = True,
    ) -> list[str]:
        """Merge setup commands with Chinese locale environment variables.

        Ensures Python/Shell scripts running in the sandbox can handle
        Chinese filenames and text content correctly.

        Args:
            setup_commands: User-provided setup commands.
            enable_chinese_locale: Whether to inject Chinese locale env vars.

        Returns:
            Combined list of setup commands.
        """
        cmds = list(setup_commands or [])
        if enable_chinese_locale:
            # Prepend locale setup so user commands inherit the env
            cmds = self.CHINESE_LOCALE_SETUP + cmds
        return cmds
    ...
    async def execute_code(
        ...
        try:
            if sandbox_id:
                ...
                # Merge Chinese locale setup with user-provided commands
                effective_setup = self._merge_setup_with_locale(
                    setup_commands, enable_chinese_locale
                )
                if effective_setup:
                    for cmd in effective_setup:
