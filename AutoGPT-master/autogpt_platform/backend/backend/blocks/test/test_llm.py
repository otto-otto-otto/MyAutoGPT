    ...
    async def test_llm_call_anthropic_returns_cache_tokens(self):
        ...
        with patch("anthropic.AsyncAnthropic") as mock_anthropic:
            ...
            response = await llm.llm_call(
                ...
                llm_model=llm.LlmModel.DEEPSEEK_V4,
                prompt=[{"role": "user", "content": "Hello"}],
                max_tokens=100,
            )
...
class TestLlmModelMissing:
    ...
    def test_provider_prefixed_model_resolves(self):
        ...
        assert (
            llm.LlmModel("z-ai/glm-4.7-flash")
            == llm.LlmModel.ZAI_GLM_4_7_FLASH
        )

    def test_bare_model_still_works(self):
        """Bare (non-prefixed) model string should still resolve correctly."""
        assert llm.LlmModel("glm-4.7-flash") == llm.LlmModel.ZAI_GLM_4_7_FLASH
...
class TestLlmModelMissingHandler:
    ...
    (``z-ai/glm-5``) that OpenRouter's OpenAI-compat endpoint
    accepts, while still validating through ``OrchestratorBlock.Input.model``
    which is typed as ``LlmModel``."""

    def test_openrouter_direct_lookup(self) -> None:
        # OpenRouter slugs containing "/" resolve via direct enum lookup.
        assert (
            llm.LlmModel("z-ai/glm-5-turbo") is llm.LlmModel.ZAI_GLM_5_TURBO
        )
        assert llm.LlmModel("z-ai/glm-5") is llm.LlmModel.ZAI_GLM_5
        assert (
            llm.LlmModel("z-ai/glm-4.7-flash")
            is llm.LlmModel.ZAI_GLM_4_7_FLASH
        )

    def test_prefix_strip_still_works(self) -> None:
        # Provider-prefixed slugs strip the prefix and resolve correctly.
        assert llm.LlmModel("extra/z-ai/glm-5") is llm.LlmModel.ZAI_GLM_5
        assert llm.LlmModel("prefix/z-ai/glm-5-turbo") is llm.LlmModel.ZAI_GLM_5_TURBO
