...
class TestExtendedThinkingProviderValidation:
    ...
    def test_real_deepseek_model_rejected(self):
        """Verify a real LlmModel enum member (DEEPSEEK_V4) fails the provider check."""
        model = LlmModel.DEEPSEEK_V4
        provider = model.metadata.provider
        assert provider not in ("anthropic", "open_router")

    def test_real_openrouter_model_fails_claude_name_check(self):
        """Verify an OpenRouter model passes provider check but fails claude name check."""
        model = LlmModel.ZAI_GLM_5
        provider = model.metadata.provider
        model_name = model.value
        assert provider in ("anthropic", "open_router")
        assert not model_name.startswith("claude")
...
class TestExtendedThinkingValidationRaisesInBlock:
    ...
    async def test_non_anthropic_provider_raises_valueerror(self):
        ...
        input_data = _make_input_data(model=LlmModel.DEEPSEEK_V4)
