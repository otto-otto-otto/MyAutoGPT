...
class StreamChatRequest(BaseModel):
    ...
    model: CopilotLlmModel | None = Field(
        default=None,
        description="Model selection: compound string like 'deepseek:advanced' or legacy 'advanced'. "
...
async def get_copilot_usage(
    ...
    Returns the percentage of the daily/weekly token allowance used — not the
    raw token counts or caps — so clients cannot derive per-turn cost or platform
    margins. Uses token-based limits (config.daily_token_limit /
    config.weekly_token_limit). Includes the user's rate-limit tier.
    """
...
async def reset_copilot_usage(
    ...
    if daily_limit <= 0:
        raise HTTPException(
            status_code=400,
            detail="No daily token limit is configured — nothing to reset.",
        )
    ...
    try:
        # Verify the user is actually at or over their daily token limit.
        usage_status = await get_token_usage_status(
            user_id=user_id,
        ...
        if daily_limit > 0 and usage_status.daily.used < daily_limit:
            raise HTTPException(
                status_code=400,
                detail="You have not reached your daily token limit yet.",
            )
        ...
        if weekly_limit > 0 and usage_status.weekly.used >= weekly_limit:
            raise HTTPException(
                status_code=400,
                detail="Your weekly token limit is also reached. Resetting the daily limit won't help.",
            )
        ...
        # Reset daily token usage in Redis.  If this fails, refund the credits
        # so the user is not charged for a service they did not receive.
        if not await reset_daily_token_usage(user_id, daily_token_limit=daily_limit):
    ...
    updated_usage = await get_token_usage_status(
        user_id=user_id,
        daily_token_limit=daily_limit,
        weekly_token_limit=weekly_limit,
        tier=tier,
...
async def stream_chat_post(
    ...
    # enforce per-window token caps. Uses config.daily_token_limit /
    ...
    if user_id:
        try:
            ...
            # past their token cap and we cannot prove otherwise. 503 + a short
