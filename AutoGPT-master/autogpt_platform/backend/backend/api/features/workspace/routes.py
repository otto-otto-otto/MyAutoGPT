...
class RenameFileRequest(BaseModel):
    name: str = Field(
        ...
        min_length=1,
        max_length=255,
        description="New filename (must not contain path separators)",
    )
...
async def rename_workspace_file(
    ...
    Updates the file name and its virtual path, keeping the directory
    prefix intact. Returns 409 if the target name already exists.
    """
    workspace = await get_workspace(user_id)
    if workspace is None:
        raise fastapi.HTTPException(status_code=404, detail="Workspace not found")
    ...
    manager = WorkspaceManager(user_id, workspace.id)
    try:
        updated = await manager.rename_file(file_id, body.name)
    except ValueError as e:
        raise fastapi.HTTPException(status_code=400, detail=str(e)) from e

    if updated is None:
        # Check whether file doesn't exist or there's a name conflict
        file = await get_workspace_file(file_id, workspace.id)
        if file is None:
            raise fastapi.HTTPException(status_code=404, detail="File not found")
        raise fastapi.HTTPException(
            status_code=409,
            detail=f"A file with name '{body.name}' already exists in this directory",
        )
