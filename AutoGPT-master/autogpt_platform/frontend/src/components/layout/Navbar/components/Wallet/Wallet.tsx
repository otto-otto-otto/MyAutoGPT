...
export function Wallet() {
  ...
  // Auto-refresh credits when a copilot chat turn completes.
  // The copilot stream hook dispatches a "copilot:credit-refresh" custom
  // event so the wallet stays in sync without the user needing to open it.
  useEffect(() => {
    const handler = () => {
      fetchCredits();
    };
    window.addEventListener("copilot:credit-refresh", handler);
    return () => window.removeEventListener("copilot:credit-refresh", handler);
  }, [fetchCredits]);
