...
import { getGetV1GetUserCreditsQueryKey } from "@/app/api/__generated__/endpoints/credits/credits";
...
interface UseCopilotStreamArgs {
  ...
  /** Vertical domain system prompt context. `null` = no override. */
...
export function useCopilotStream({
  ...
  if (chatRuntime) {
    ...
    chatRuntime.domainContextRef.current = domainContext;
  }
  ...
  useEffect(() => {
    ...
    if (wasActive && isIdle && sessionId && !isReconnectScheduled) {
      ...
      queryClient.invalidateQueries({
        queryKey: getGetV1GetUserCreditsQueryKey(),
      });
      // Refresh the wallet credit display so the user sees the deduction
      // without having to manually open the wallet popover.
      window.dispatchEvent(new CustomEvent("copilot:credit-refresh"));
    }
