...
let mockCopilotLlmModel = "deepseek:standard";
...
afterEach(() => {
  ...
  mockCopilotLlmModel = "deepseek:standard";
  mockFlagValue = false;
});
...
describe("ChatInput model toggle", () => {
  ...
  it("does not render model selector when flag is disabled", () => {
    mockFlagValue = false;
    render(<ChatInput onSend={mockOnSend} />);
    expect(
      screen.queryByLabelText(/open model selector/i),
    ).toBeNull();
  });

  it("opens model selector and selects qwen:standard", () => {
    mockFlagValue = true;
    mockCopilotLlmModel = "deepseek:standard";
    render(<ChatInput onSend={mockOnSend} />);
    fireEvent.click(screen.getByLabelText(/open model selector/i));
    fireEvent.click(screen.getByText("通义千问"));
    expect(mockSetCopilotLlmModel).toHaveBeenCalledWith("qwen:standard");
  });

  it("opens model selector and selects deepseek:advanced", () => {
    mockFlagValue = true;
    mockCopilotLlmModel = "deepseek:standard";
    render(<ChatInput onSend={mockOnSend} />);
    fireEvent.click(screen.getByLabelText(/open model selector/i));
    // Click on the tier toggle to switch to advanced
    const tierToggle = screen.getByRole("switch");
    fireEvent.click(tierToggle);
    expect(mockSetCopilotLlmModel).toHaveBeenCalledWith("deepseek:advanced");
  });

  it("hides model selector when streaming", () => {
    mockFlagValue = true;
    render(<ChatInput onSend={mockOnSend} isStreaming />);
    expect(
      screen.queryByLabelText(/open model selector/i),
  ...
  it("shows model selector when hasSession is true and not streaming", () => {
  ...
  it("shows a toast when switching to qwen:advanced", async () => {
    const { toast } = await import("@/components/molecules/Toast/use-toast");
    mockFlagValue = true;
    mockCopilotLlmModel = "qwen:standard";
    render(<ChatInput onSend={mockOnSend} />);
    fireEvent.click(screen.getByLabelText(/open model selector/i));
    // Click tier toggle to switch to advanced
    fireEvent.click(screen.getByRole("switch"));
    expect(toast).toHaveBeenCalledWith(
      expect.objectContaining({
        title: expect.stringMatching(/通义千问/),
      }),
    );
  });

  it("shows a toast when switching to deepseek:standard", async () => {
    const { toast } = await import("@/components/molecules/Toast/use-toast");
    mockFlagValue = true;
    mockCopilotLlmModel = "qwen:advanced";
    render(<ChatInput onSend={mockOnSend} />);
    fireEvent.click(screen.getByLabelText(/open model selector/i));
    // Click DeepSeek button
    fireEvent.click(screen.getByText("DeepSeek"));
    expect(toast).toHaveBeenCalledWith(
      expect.objectContaining({
        title: expect.stringMatching(/DeepSeek/),
      }),
    );
  });
});
