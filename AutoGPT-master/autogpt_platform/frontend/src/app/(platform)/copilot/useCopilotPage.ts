...
import { useCopilotUIStore, VERTICAL_DOMAIN_PROMPTS } from "./store";
...
export function useCopilotPage() {
  ...
  const { copilotChatMode, copilotLlmModel, isDryRun, selectedDomain } =
    useCopilotUIStore();
  const domainContext = useMemo(() => {
    if (!selectedDomain) return null;
    return VERTICAL_DOMAIN_PROMPTS[selectedDomain] ?? null;
  }, [selectedDomain]);
