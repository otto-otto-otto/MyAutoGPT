...
describe("useCopilotUIStore", () => {
  beforeEach(() => {
    ...
    useCopilotUIStore.setState({
      ...
      copilotLlmModel: "deepseek:standard",
    });
  });
  ...
  describe("copilotLlmModel", () => {
    it("defaults to deepseek:standard", () => {
      expect(useCopilotUIStore.getState().copilotLlmModel).toBe("deepseek:standard");
    });

    it("sets model to deepseek:advanced", () => {
      useCopilotUIStore.getState().setCopilotLlmModel("deepseek:advanced");
      expect(useCopilotUIStore.getState().copilotLlmModel).toBe("deepseek:advanced");
    });

    it("persists model to localStorage", () => {
      useCopilotUIStore.getState().setCopilotLlmModel("deepseek:advanced");
      expect(window.localStorage.getItem("copilot-model")).toBe("deepseek:advanced");
    });
  });
  ...
  describe("clearCopilotLocalData", () => {
    it("resets state and clears localStorage keys", () => {
      ...
      expect(state.copilotLlmModel).toBe("deepseek:standard");
...
describe("useCopilotUIStore localStorage initialisation", () => {
  ...
  it("reads deepseek:advanced model from localStorage on store creation", async () => {
    window.localStorage.setItem("copilot-model", "deepseek:advanced");
    vi.resetModules();
    const { useCopilotUIStore: fresh } = await import("../store");
    expect(fresh.getState().copilotLlmModel).toBe("deepseek:advanced");
  });
});
