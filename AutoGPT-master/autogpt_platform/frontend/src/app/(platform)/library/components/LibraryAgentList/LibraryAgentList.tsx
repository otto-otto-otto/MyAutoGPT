...
import type { LibraryTab, AgentStatusFilter, FleetSummary, DomainId } from "../../types";
...
interface Props {
  ...
  domainFilteredAgents?: LibraryAgent[];
...
export function LibraryAgentList({
  ...
  const agents = useMemo(() => {
    if (!selectedDomain) return rawAgents;
    const targetGraphId = DOMAIN_GRAPH_ID_MAP[selectedDomain];
    return rawAgents.filter((agent) => agent.graph_id === targetGraphId);
  }, [rawAgents, selectedDomain]);
